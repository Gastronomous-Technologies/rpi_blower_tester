...
def get_firmware_version(thermal_monitor):
    err = True
    ...
    conf["log"].error("Timeout, could not communicate over SPI")

    return err

def get_hdc_temp(thermal_monitor):
    thermal_monitor.request_packet()
    conf["log"].debug(f"Packet Received: \n{thermal_monitor.packet}")
    ...
    hdc_temp = thermal_monitor.packet.temp_board_C

    return hdc_temp

def get_tc_temp(thermal_monitor, tc_ch):
    if tc_ch not in conf["tc"]["range"]:
        conf["log"].exception("Error, desired thermocouple number out of range")
        raise ValueError
    else:
        thermal_monitor.request_packet()
        conf["log"].debug(f"Packet Received: \n{thermal_monitor.packet}")
...
def get_fan_speed(thermal_monitor, fan_num):
    ...
    if fan_num not in conf["fan"]["range"]:
        conf["log"].exception("Error, desired fan number out of range")
        raise ValueError
    else:
        packet = TMStatusPacket() 
        thermal_monitor.request_packet()
